...
try {
   ...
   rs2 = stmt.executeQuery("select * from items where category = 'Eggs, Meat and Fish' and stock > 0");
} 
   catch (Exception e){
    ...
    if(!con.isClosed())
    {
    	...
    	while(rs1.next())
    	{
    		if(name != "" && name.equals(rs1.getString("name")))
    		{
    			...
    			response.sendRedirect("meat.jsp");
    			break;
    		}
    	}
    	if(k == 0)
...
<div id = "blur">
        ...
        <div class = "type">
            <div class = "type-image">
                <p style = "color:white">Eggs, Meat and Fish</p>
            </div>
        </div>
        <div class = "list">
            <% while(rs2.next()){%>
