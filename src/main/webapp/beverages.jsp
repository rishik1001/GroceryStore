...
<%
	...
	{
		session.setAttribute("taken",Integer.parseInt(request.getParameter("items")));
		session.setAttribute("pre",0);
	}
	Class.forName("com.mysql.jdbc.Driver");
	String url1 = "jdbc:mysql://localhost:3306/itmini?useSSL=false&useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC";
    String user1 = "root";
    Statement st1 = null;
    String mysqlpassword1 = "Rishik@123";
    Connection connection = DriverManager.getConnection(url1, user1, mysqlpassword1);
    if(!connection.isClosed())
    {
    	st1 = connection.createStatement();
    	rs2 = st1.executeQuery("select * from items where category = 'Beverages' and stock > 0 limit "+session.getAttribute("taken")+" offset "+session.getAttribute("pre"));
    	session.setAttribute("pre",Integer.parseInt((String)session.getAttribute("pre")) + Integer.parseInt((String)session.getAttribute("taken")));
    }
%>
<%
if(session.getAttribute("username") == null)
{
	out.println("<html><head></head><body onload = \"alert('Sign In Required');window.loction='register.jsp';\"></body></html>");
}
    ...
    if(!con.isClosed())
    {
    	...
    	while(rs1.next())
    	{
    		if(name != "" && name.equals(rs1.getString("name")))
    		{
    			...
    			response.sendRedirect("beverages.jsp");
    			break;
    		}
    	}
    	if(k == 0)
...
<body>
      <div id = "blur">
        <div class = "bar">
         ...
         <div class = "type">
            ...
            <div>
             <form method = "post" action = "beverages.jsp">
             <label style = "color: white">Items per page: </label>
             <input type = "text" name = "items" style = "width: 30px"><br>
             <a href="beverages.jsp" class="buttonClass">Apply</a>
             </form>
            </div>
        </div><br>
        <div class = "list">
    ...
    function toggle() {
        ...
        if(session.getAttribute("username") == null)
        {
        	...
        	var register = document.getElementById('popup');
